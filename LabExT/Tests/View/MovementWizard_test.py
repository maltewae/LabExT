...
class MovementWizardAssignmentStepTest(TKinterTestCase):
    ...
    def test_position_selection_without_orientation_selection_before(self):
        self.assertIsNone(self.view._current_stage_assignment.get(self.stage))

        self.view.stage_position_var[self.stage].set(DevicePosition.INPUT)
        self.assertIsNone(self.view._current_stage_assignment.get(self.stage))

    def test_position_selection_with_orientation_selection(self):
        self.assertIsNone(self.view._current_stage_assignment.get(self.stage))

        self.view.stage_orientation_var[self.stage].set(Orientation.BOTTOM)
        self.view.stage_position_var[self.stage].set(DevicePosition.OUTPUT)
        self.assertEqual(self.view._current_stage_assignment.get(
            self.stage), (Orientation.BOTTOM, DevicePosition.OUTPUT))

        self.view.stage_orientation_var[self.stage].set(Orientation.TOP)
        self.view.stage_position_var[self.stage].set(DevicePosition.INPUT)
        self.assertEqual(self.view._current_stage_assignment.get(
            self.stage), (Orientation.TOP, DevicePosition.INPUT))
    ...
    def test_assignment_valid(self):
        ...
        self.view.stage_position_var[self.stage2].set(DevicePosition.INPUT)
    ...
    def test_invalid_assignment_with_double_orientation(self):
        ...
        self.assertEqual(
            self.view._error_label["text"],
            "Please do not assign a orientation or device position twice.")
        self.assertEqual(self.view._next_button["state"], DISABLED)
    ...
    def test_invalid_assignment_with_double_position(self):
        self.view.stage_orientation_var[self.stage].set(Orientation.BOTTOM)
        self.view.stage_orientation_var[self.stage2].set(Orientation.TOP)
        ...
        self.view.stage_position_var[self.stage].set(DevicePosition.OUTPUT)
        self.view.stage_position_var[self.stage2].set(DevicePosition.OUTPUT)
        self.assertEqual(
            self.view._error_label["text"],
            "Please do not assign a orientation or device position twice.")
        self.assertEqual(self.view._next_button["state"], DISABLED)
...
class MovementWizardControllerTest(unittest.TestCase):
    ...
    def test_save_one_stage(self):
        self.controller.save(
            {self.stage: (Orientation.LEFT, DevicePosition.INPUT)})
    ...
    def test_save_double_position(self):
        with self.assertRaises(MoverError):
            self.controller.save({
                self.stage: (Orientation.LEFT, DevicePosition.INPUT),
                self.stage2: (Orientation.RIGHT, DevicePosition.INPUT),
            })
    ...
    def test_save_double_orientation(self):
        with self.assertRaises(MoverError):
            self.controller.save({
                self.stage: (Orientation.LEFT, DevicePosition.INPUT),
                self.stage2: (Orientation.LEFT, DevicePosition.OUTPUT),
            })
    ...
    def test_save_sets_stage_settings(self):
        self.controller.save(
            stage_assignment={
                self.stage: (Orientation.LEFT, DevicePosition.INPUT),
                self.stage2: (Orientation.RIGHT, DevicePosition.OUTPUT)
            },
            speed_xy=300,
            speed_z=400,
            acceleration_xy=100
...
class MovementWizardIntegrationTest(TKinterTestCase):
    ...
    def test_setup_of_two_stages(
        ...
        wizard.stage_position_var[stage].set(DevicePosition.OUTPUT)

        wizard.stage_orientation_var[stage2].set(Orientation.TOP)
        wizard.stage_position_var[stage2].set(DevicePosition.OUTPUT)
        ...
        wizard.stage_position_var[stage2].set(DevicePosition.INPUT)
        ...
        with patch("LabExT.View.MovementWizard.MovementWizardView.messagebox.showinfo", autospec=True) as messagebox_mock:
            ...
            self.assertEqual(self.mover.calibrations[(
                Orientation.LEFT, DevicePosition.OUTPUT)].stage, stage)
            self.assertEqual(self.mover.calibrations[(
                Orientation.RIGHT, DevicePosition.INPUT)].stage, stage2)
