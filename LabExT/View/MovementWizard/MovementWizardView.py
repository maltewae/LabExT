...
class MovementWizardView(Wizard):
    ...
    def __init__(self, parent, controller, mover):
        ...
        self.stage_position_var = {}
        self.stage_orientation_var = {}
        for stage in self.mover.available_stages:
            self.stage_position_var.update(
                {stage: StringVar(self.parent, DevicePosition.INPUT)})
            self.stage_position_var[stage].trace(
    ...
    def _stage_assignment_step_builder(self, frame: Type[CustomFrame]):
        ...
        for available_stage in self.mover.available_stages:
            ...
            # Set up menu for position selection
            stage_position_menu = OptionMenu(
                available_stage_frame,
                self.stage_position_var[available_stage],
                *(list(DevicePosition))
            )
            stage_position_menu.pack(side=RIGHT, padx=5)
            stage_position_menu.config(state=DISABLED if self.stage_orientation_var[available_stage].get(
            ) == self.ASSIGNMENT_MENU_PLACEHOLDER else NORMAL)

            Label(
                available_stage_frame, text="Device Position:"
    ...
    def _on_stage_assignment(self, stage):
        ...
        """
        position = self.stage_position_var.get(stage, StringVar()).get()
        ...
        self._current_stage_assignment[stage] = (
            Orientation[orientation.upper()], DevicePosition[position.upper()])
        self.__reload__()
    ...
    def _is_assignment_valid(self) -> Tuple[bool, str]:
        ...
        Checks for duplicate orientations and/or positions.
        """
